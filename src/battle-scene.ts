...
export default class BattleScene extends SceneBase {
  ...
  * - false: Show hints for moves
  ...
  addPlayerPokemon(species: PokemonSpecies, level: integer, formIndex?: integer, gender?: Gender, shiny?: boolean, variant?: Variant, ivs?: integer[], dataSource?: Pokemon | PokemonData, postProcess?: (playerPokemon: PlayerPokemon) => void): PlayerPokemon {
    const pokemon = new PlayerPokemon(this, species, level, formIndex, gender, shiny, variant, ivs, dataSource);
    if (postProcess) {
      postProcess(pokemon);
    }
    pokemon.init();
    ...
    if (this.gameMode.isFixedBattle(newWaveIndex) && trainerData === undefined) {
      ...
      if (newBattleType === BattleType.TRAINER) {
        ...
        if (trainerConfigs[trainerType].doubleOnly) {
          ...
          if (trainerConfigs[trainerType].trainerTypeDouble) {
            doubleTrainer = false;
          }
        }
  ...
  initPokemonSprite(sprite: Phaser.GameObjects.Sprite, pokemon?: Pokemon, hasShadow: boolean = false, ignoreOverride: boolean = false): Phaser.GameObjects.Sprite {
    sprite.setPipeline(this.spritePipeline, { tone: [ 0.0, 0.0, 0.0, 0.0 ], hasShadow: hasShadow, ignoreOverride: ignoreOverride });
  ...
  triggerPokemonFormChange(pokemon: Pokemon, formChangeTriggerType: Constructor<SpeciesFormChangeTrigger>, delayed: boolean = false, modal: boolean = false): boolean {
    if (pokemonFormChanges.hasOwnProperty(pokemon.species.speciesId)) {
      ...
      if (matchingFormChangeOpts.length > 1) {
